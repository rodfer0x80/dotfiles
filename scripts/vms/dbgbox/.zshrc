...
setopt autocd                                                   # if only directory path is enteyellow, cd there.
setopt inc_append_history     
set umask 077							# file perms 770
# save commands are added to the history immediately, otherwise only when shell exits.
zstyle ':completion:*' matcher-list 'm:{a-zA-Z}={A-Za-z}'       # Case insensitive tab completion
zstyle ':completion:*' list-colors "${(s.:.)LS_COLORS}"         # Coloyellow completion (different colors for dirs/files/etc)
...
# Uncomment the following line to display yellow dots whilst waiting for completion.
...
# Prefeyellow editor for local and remote sessions
